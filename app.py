...
THEORETICAL_PERF = {
    ...
    "Deflazione":      {"Utilities":5,"Healthcare":4,"Beni di prima necessità":4,"Comunicazioni":3,"Real Estate":1,"Tecnologia":0,"Finanziari":-1,"Industriali":-2,"Materiali":-3,"Energia":-4,"Ciclici":-3},
...
/* Section title (usato in settori, screening, fairvalue) */
.section-title{font-family:'DM Mono',monospace;font-size:11px;color:#b8922a;letter-spacing:.15em;
               text-transform:uppercase;border-bottom:1px solid #e5e7eb;padding-bottom:8px;margin:24px 0 14px;}
...
def _regime_score(gdp, inf, un, ca=None, debt=None, gdp_trend=None, inf_trend=None):
    ...
    if gdp_trend is not None and gdp_trend > 0: s += 8
    if inf_trend is not None and inf_trend < 0: s += 7
    ...
    if gdp_trend is not None and gdp_trend > 0: s += 5
    if inf_trend is not None and inf_trend > 0: s += 10
    ...
    if gdp_trend is not None and gdp_trend < 0: s += 5
    if inf_trend is not None and inf_trend > 0: s += 5
    ...
    if gdp_trend is not None and gdp_trend < -0.5: s += 10
    ...
    if gdp_trend is not None and gdp_trend > 0.5: s += 15   # il trend in miglioramento è cruciale
    ...
    if gdp_trend is not None and gdp_trend < 0: s += 10
...
def _page_macro():
    ...
    if gv is not None and iv is not None and uv is not None:
        ...
        st.rerun()  # sblocca navbar immediatamente
        ...
        st.markdown('<div class="success">✅ Regime identificato. Usa la navbar in alto per navigare ai Settori.</div>', unsafe_allow_html=True)
    else:
    ...
    # ── SENTIMENT DI MERCATO (Fear & Greed) ──────────────────────────────────
    ...
    fg_icon  = ("😱" if (fg_score or 50)<25 else "😰" if (fg_score or 50)<45
                else "😐" if (fg_score or 50)<55 else "😄" if (fg_score or 50)<75 else "🤑")
    fg_label_map = {
        "extreme fear":"Paura Estrema","fear":"Paura","neutral":"Neutro",
        "greed":"Avidità","extreme greed":"Avidità Estrema"
    }
    fg_label_ita = fg_label_map.get(fg_label.lower(), fg_label.upper())
    fg_col, fg_rest = st.columns([1,3])
    with fg_col:
        st.markdown(f'''<div class="card" style="text-align:center;">
            <div class="label">CNN Fear &amp; Greed Index</div>
            <div style="font-size:40px;margin:6px 0;">{fg_icon}</div>
            <div style="font-family:DM Serif Display,serif;font-size:32px;color:{fg_color};">{fg_score if fg_score is not None else "N/D"}</div>
            <div style="font-family:DM Mono;font-size:12px;color:{fg_color};margin-top:4px;">{fg_label_ita}</div>
        </div>''', unsafe_allow_html=True)
    with fg_rest:
        st.markdown('''<div class="info">
        <b>Come leggere il Fear &amp; Greed Index:</b><br>
        0-24 = Paura Estrema (possibile opportunità di acquisto) &nbsp;·&nbsp;
        25-44 = Paura &nbsp;·&nbsp; 45-55 = Neutro &nbsp;·&nbsp;
        56-74 = Avidità &nbsp;·&nbsp; 75-100 = Avidità Estrema (mercato surriscaldato).
        </div>''', unsafe_allow_html=True)
        ...
        with cols[i%4]:
            st.markdown(f'''<div class="card" style="padding:12px 14px;">
                ...
                <div style="font-family:DM Mono;font-size:11px;color:{chg_color};margin-top:3px;">
                    {"▲" if (chg or 0)>=0 else "▼"} {abs(chg or 0):.2f}% oggi
...
# ── Dati reali FRED per calendario (USA) ─────────────────────────────────────
@st.cache_data(ttl=3600*4, show_spinner=False)
def _fred_latest(series_id: str) -> tuple:
    """Scarica da FRED il valore più recente e il precedente per un indicatore."""
    try:
        ...
        if r.status_code != 200:
            return None, None
        import io as _io
        df = pd.read_csv(_io.StringIO(r.text), names=["date","value"], skiprows=1)
        df["value"] = pd.to_numeric(df["value"], errors="coerce")
        df = df.dropna().tail(3)
        if len(df) < 2:
            return None, None
        actual   = round(float(df.iloc[-1]["value"]), 2)
        previous = round(float(df.iloc[-2]["value"]), 2)
        return actual, previous
    except:
        return None, None

@st.cache_data(ttl=3600*4, show_spinner=False)
def _wb_latest(cc: str, indicator: str) -> tuple:
    """Scarica da World Bank il valore più recente e il precedente."""
    try:
        url = f"https://api.worldbank.org/v2/country/{cc}/indicator/{indicator}?format=json&per_page=5&mrv=5"
        r = requests.get(url, timeout=10)
        if r.status_code != 200:
            return None, None
        p = r.json()
        if len(p) < 2 or not p[1]:
            return None, None
        vals = [d["value"] for d in p[1] if d["value"] is not None]
        if len(vals) < 2:
            return None, None
...
@st.cache_data(ttl=3600*4, show_spinner=False)
def _imf_latest(cc: str, indicator: str) -> tuple:
    """Scarica da IMF WEO il valore più recente e il precedente."""
    try:
        iso3 = ISO2_TO_ISO3.get(cc)
        if not iso3:
            return None, None
        url = f"https://www.imf.org/external/datamapper/api/v1/{indicator}/{iso3}?periods=2022,2023,2024,2025"
        r = requests.get(url, timeout=10)
        if r.status_code != 200:
            return None, None
        data   = r.json()
        values = data.get("values",{}).get(indicator,{}).get(iso3,{})
        vals   = [(int(k), float(v)) for k,v in values.items() if v is not None]
        vals   = sorted(vals, key=lambda x: x[0], reverse=True)
        if len(vals) < 2:
            return None, None
        return round(vals[0][1], 2), round(vals[1][1], 2)
    except:
        return None, None
...
def get_calendar_events(cc: str) -> list:
    """
    Recupera i valori reali degli indicatori macro da FRED (USA),
    World Bank e IMF per tutti gli altri paesi.
    Mostra ATTUALE, PRECEDENTE e stima PREVISTO (media storica come proxy).
    """
    now = datetime.now()
    ...
    if cc == "US":
        # ── USA: FRED ──────────────────────────────────────────────────
        # CPI YoY (CPIAUCSL → calcoliamo variazione %)
        try:
            url = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=CPIAUCSL"
            r = requests.get(url, timeout=10)
            if r.status_code == 200:
                import io as _io
                df_cpi = pd.read_csv(_io.StringIO(r.text), names=["date","value"], skiprows=1)
                df_cpi["value"] = pd.to_numeric(df_cpi["value"], errors="coerce")
                df_cpi = df_cpi.dropna()
                if len(df_cpi) >= 13:
                    act_cpi  = round((df_cpi["value"].iloc[-1]/df_cpi["value"].iloc[-13]-1)*100, 1)
                    prev_cpi = round((df_cpi["value"].iloc[-2]/df_cpi["value"].iloc[-14]-1)*100, 1)
                    # Forecast: media ultimi 3 mesi come proxy
                    hist_cpi = [round((df_cpi["value"].iloc[-i]/df_cpi["value"].iloc[-i-12]-1)*100, 1) for i in range(1,4)]
                    fcast_cpi = round(sum(hist_cpi)/len(hist_cpi), 1)
                    results.append({
                        "event":"🇺🇸 Inflazione CPI (YoY)","impact":"Alto",
                        "actual":f"{act_cpi:.1f}%","forecast":f"{fcast_cpi:.1f}%","previous":f"{prev_cpi:.1f}%",
                        "freq":"Mensile","when":"~12 del mese","date":month_str,
                    })
        except: pass

        # Disoccupazione (UNRATE)
        act_un, prev_un = _fred_latest("UNRATE")
        results.append({
            "event":"🇺🇸 Tasso Disoccupazione","impact":"Alto",
            "actual":_fmt_val(act_un,"%"),"forecast":"—","previous":_fmt_val(prev_un,"%"),
            "freq":"Mensile","when":"1° venerdì del mese","date":month_str,
        })

        # PIL trimestrale (A191RL1Q225SBEA = Real GDP growth QoQ annualizzato)
        act_gdp, prev_gdp = _fred_latest("A191RL1Q225SBEA")
        results.append({
            "event":"🇺🇸 PIL reale (QoQ ann.)","impact":"Alto",
            "actual":_fmt_val(act_gdp,"%"),"forecast":"—","previous":_fmt_val(prev_gdp,"%"),
            "freq":"Trimestrale","when":"Fine mese","date":month_str,
        })

        # Fed Funds Rate (FEDFUNDS)
        act_ff, prev_ff = _fred_latest("FEDFUNDS")
        results.append({
            "event":"🇺🇸 Fed Funds Rate","impact":"Alto",
            "actual":_fmt_val(act_ff,"%"),"forecast":"—","previous":_fmt_val(prev_ff,"%"),
            "freq":"8x anno","when":"Vedi Fed calendar","date":month_str,
        })

        # Retail Sales MoM (RSXFS → calcoliamo MoM %)
        act_rs, prev_rs = _fred_latest("RSXFS")
        if act_rs and prev_rs:
            # RSXFS è in livelli → convertiamo in MoM%
            try:
                url2 = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=RSXFS"
                r2 = requests.get(url2, timeout=10)
                import io as _io2
                df_rs = pd.read_csv(_io2.StringIO(r2.text), names=["date","value"], skiprows=1)
                df_rs["value"] = pd.to_numeric(df_rs["value"], errors="coerce")
                df_rs = df_rs.dropna()
                if len(df_rs) >= 3:
                    rs_act  = round((df_rs["value"].iloc[-1]/df_rs["value"].iloc[-2]-1)*100, 1)
                    rs_prev = round((df_rs["value"].iloc[-2]/df_rs["value"].iloc[-3]-1)*100, 1)
                    results.append({
                        "event":"🇺🇸 Retail Sales (MoM)","impact":"Medio",
                        "actual":f"{rs_act:+.1f}%","forecast":"—","previous":f"{rs_prev:+.1f}%",
                        "freq":"Mensile","when":"~15 del mese","date":month_str,
                    })
            except: pass

        # Initial Jobless Claims (ICSA)
        act_ic, prev_ic = _fred_latest("ICSA")
        results.append({
            "event":"🇺🇸 Richieste sussidi disoccupazione","impact":"Medio",
            "actual":f"{int(act_ic):,}" if act_ic else "—",
            "forecast":"—",
            "previous":f"{int(prev_ic):,}" if prev_ic else "—",
            "freq":"Settimanale","when":"Ogni giovedì","date":month_str,
        })

    else:
        # ── NON-USA: IMF + World Bank ──────────────────────────────────

        # PIL (IMF NGDP_RPCH = crescita PIL reale %)
        act_gdp, prev_gdp = _imf_latest(cc, "NGDP_RPCH")
        results.append({
            "event":f"PIL reale (crescita %)","impact":"Alto",
            "actual":_fmt_val(act_gdp,"%"),"forecast":"—","previous":_fmt_val(prev_gdp,"%"),
            "freq":"Trimestrale","when":"Fine trimestre","date":str(now.year),
        })

        # Inflazione (IMF PCPIPCH = CPI % change)
        act_inf, prev_inf = _imf_latest(cc, "PCPIPCH")
        results.append({
            "event":f"Inflazione CPI (YoY %)","impact":"Alto",
            "actual":_fmt_val(act_inf,"%"),"forecast":"—","previous":_fmt_val(prev_inf,"%"),
            "freq":"Mensile","when":"~20 del mese","date":month_str,
        })

        # Disoccupazione (IMF LUR)
        act_un, prev_un = _imf_latest(cc, "LUR")
        results.append({
            "event":f"Tasso Disoccupazione (%)","impact":"Alto",
            "actual":_fmt_val(act_un,"%"),"forecast":"—","previous":_fmt_val(prev_un,"%"),
            "freq":"Mensile","when":"Fine mese","date":month_str,
        })

        # Debito pubblico (World Bank)
        act_dbt, prev_dbt = _wb_latest(cc, "GC.DOD.TOTL.GD.ZS")
        results.append({
            "event":f"Debito pubblico (% PIL)","impact":"Medio",
            "actual":_fmt_val(act_dbt,"%"),"forecast":"—","previous":_fmt_val(prev_dbt,"%"),
            "freq":"Annuale","when":"Fine anno","date":str(now.year),
        })

        # Conto corrente (World Bank)
        act_ca, prev_ca = _wb_latest(cc, "BN.CAB.XOKA.GD.ZS")
        results.append({
            "event":f"Conto Corrente (% PIL)","impact":"Medio",
            "actual":_fmt_val(act_ca,"%"),"forecast":"—","previous":_fmt_val(prev_ca,"%"),
            "freq":"Trimestrale","when":"Fine trimestre","date":str(now.year),
        })

        # Aggiungi eventi fissi specifici per area
        FIXED = {
            "DE":[
                ...
                {"event":"🇩🇪 PMI Manifatturiero",      "impact":"Medio","freq":"Mensile","when":"1° gg lavorativo"},
            ],
            "IT":[
                {"event":"🇮🇹 BCE Riunione tassi",       "impact":"Alto","freq":"8x anno","when":"Vedi BCE calendar"},
                {"event":"🇮🇹 PMI Manifatturiero Italia","impact":"Medio","freq":"Mensile","when":"1° gg lavorativo"},
                {"event":"🇮🇹 Inflazione Eurozona",      "impact":"Alto","freq":"Mensile","when":"~31 del mese"},
            ],
            "FR":[
                {"event":"🇫🇷 BCE Riunione tassi",       "impact":"Alto","freq":"8x anno","when":"Vedi BCE calendar"},
                {"event":"🇫🇷 PMI Composito Francia",    "impact":"Medio","freq":"Mensile","when":"1° gg lavorativo"},
            ],
            "GB":[
                {"event":"🇬🇧 BoE Riunione tassi",       "impact":"Alto","freq":"8x anno","when":"Vedi BoE calendar"},
                {"event":"🇬🇧 PMI Manifatturiero UK",     "impact":"Medio","freq":"Mensile","when":"1° gg lavorativo"},
            ],
            "JP":[
                {"event":"🇯🇵 BoJ Riunione tassi",        "impact":"Alto","freq":"8x anno","when":"Vedi BoJ calendar"},
                {"event":"🇯🇵 Tankan Survey",             "impact":"Alto","freq":"Trimestrale","when":"Inizio trimestre"},
            ],
        }
        for ev in FIXED.get(cc, []):
            results.append({
                "event":ev["event"],"impact":ev["impact"],
                "actual":"—","forecast":"—","previous":"—",
                "freq":ev["freq"],"when":ev.get("when",""),"date":"Ricorrente",
            })

    return results if results else _static_calendar(cc)

def _static_calendar(cc: str) -> list:
    """Fallback statico minimale."""
    now = datetime.now()
    BASE = [
        {"event":"PIL reale (crescita %)","impact":"Alto","freq":"Trimestrale","when":"Fine trimestre"},
        {"event":"Inflazione CPI (YoY)","impact":"Alto","freq":"Mensile","when":"~20 del mese"},
        {"event":"Tasso Disoccupazione","impact":"Alto","freq":"Mensile","when":"Fine mese"},
        {"event":"Tasso di interesse banca centrale","impact":"Alto","freq":"8x anno","when":"Vedi calendario banca centrale"},
        {"event":"PMI Manifatturiero","impact":"Medio","freq":"Mensile","when":"1° gg lavorativo"},
    ]
    return [{"event":e["event"],"impact":e["impact"],"actual":"—","forecast":"—",
             "previous":"—","freq":e["freq"],"when":e.get("when",""),
             "date":str(now.year)} for e in BASE]
...
def _page_sectors():
    ...
    if reg is None:
        st.warning("⚠️ Completa prima la sezione Macro per identificare il regime.")
        return
    reg_name     = reg["name"]
...
# ── MAPPING SETTORI → GICS (globale, usato da screening) ─────────────────────
SECTOR_TO_GICS_MAP = {
    "Tecnologia":              "Information Technology",
    "Finanziari":              "Financials",
    "Energia":                 "Energy",
...
# Curated stocks per paese non-US (usati come fallback se Wikipedia fallisce)
CURATED_STOCKS_SCREENING = {
    "IT": [
        ("ENI.MI","Eni","Energia"),("ENEL.MI","Enel","Utilities"),
        ("ISP.MI","Intesa Sanpaolo","Finanziari"),("UCG.MI","UniCredit","Finanziari"),
        ("STM.MI","STMicroelectronics","Tecnologia"),("LDO.MI","Leonardo","Industriali"),
    ...
    "JP": [
        ("7203.T","Toyota","Ciclici"),("6758.T","Sony","Tecnologia"),
        ("9984.T","SoftBank","Tecnologia"),("8306.T","Mitsubishi UFJ","Finanziari"),
        ("7267.T","Honda","Ciclici"),("6861.T","Keyence","Tecnologia"),
        ("9432.T","NTT","Comunicazioni"),("8058.T","Mitsubishi Corp","Industriali"),
        ("4502.T","Takeda","Healthcare"),("6367.T","Daikin","Industriali"),
    ],
    "CN": [
        ("0700.HK","Tencent","Tecnologia"),("BABA","Alibaba","Ciclici"),
        ("9988.HK","Alibaba HK","Ciclici"),("3690.HK","Meituan","Tecnologia"),
        ("9618.HK","JD.com","Ciclici"),("0939.HK","CCB","Finanziari"),
        ("0941.HK","China Mobile","Comunicazioni"),("2318.HK","Ping An","Finanziari"),
    ],
    "BR": [
        ("PETR4.SA","Petrobras","Energia"),("VALE3.SA","Vale","Materiali"),
        ("ITUB4.SA","Itaú Unibanco","Finanziari"),("BBDC4.SA","Bradesco","Finanziari"),
        ("ABEV3.SA","Ambev","Beni di prima necessità"),("B3SA3.SA","B3","Finanziari"),
        ("WEGE3.SA","WEG","Industriali"),("MGLU3.SA","Magazine Luiza","Ciclici"),
    ],
    "IN": [
        ("RELIANCE.NS","Reliance Industries","Energia"),("TCS.NS","TCS","Tecnologia"),
        ("HDFCBANK.NS","HDFC Bank","Finanziari"),("INFY.NS","Infosys","Tecnologia"),
        ("HINDUNILVR.NS","HUL","Beni di prima necessità"),("ICICIBANK.NS","ICICI Bank","Finanziari"),
        ("WIPRO.NS","Wipro","Tecnologia"),("BAJFINANCE.NS","Bajaj Finance","Finanziari"),
        ("SUNPHARMA.NS","Sun Pharma","Healthcare"),("MARUTI.NS","Maruti Suzuki","Ciclici"),
    ],
    "CA": [
        ("RY.TO","Royal Bank","Finanziari"),("TD.TO","TD Bank","Finanziari"),
        ("ENB.TO","Enbridge","Energia"),("CNQ.TO","CNQ","Energia"),
        ("CP.TO","CP Rail","Industriali"),("BMO.TO","BMO","Finanziari"),
        ("SU.TO","Suncor","Energia"),("BCE.TO","BCE","Comunicazioni"),
        ("SHOP.TO","Shopify","Tecnologia"),("ABX.TO","Barrick Gold","Materiali"),
    ],
    "AU": [
        ("CBA.AX","Commonwealth Bank","Finanziari"),("BHP.AX","BHP","Materiali"),
        ("CSL.AX","CSL","Healthcare"),("NAB.AX","NAB","Finanziari"),
        ("WBC.AX","Westpac","Finanziari"),("ANZ.AX","ANZ","Finanziari"),
        ("WES.AX","Wesfarmers","Ciclici"),("RIO.AX","Rio Tinto","Materiali"),
        ("MQG.AX","Macquarie","Finanziari"),("TLS.AX","Telstra","Comunicazioni"),
    ],
    "KR": [
        ("005930.KS","Samsung Electronics","Tecnologia"),("000660.KS","SK Hynix","Tecnologia"),
        ("051910.KS","LG Chem","Materiali"),("035420.KS","NAVER","Tecnologia"),
        ("005380.KS","Hyundai Motor","Ciclici"),("055550.KS","Shinhan Financial","Finanziari"),
        ("000270.KS","Kia","Ciclici"),("096770.KS","SK Innovation","Energia"),
    ],
}

@st.cache_data(ttl=3600*6, show_spinner=False)
def get_sp500_tickers_global() -> pd.DataFrame:
    """S&P 500 da Wikipedia con settore GICS. Cache 6h a livello globale."""
    try:
        tables = pd.read_html("https://en.wikipedia.org/wiki/List_of_S%26P_500_companies")
        df = tables[0][["Symbol","Security","GICS Sector"]]
        df.columns = ["ticker","name","sector_raw"]
...
@st.cache_data(ttl=3600*6, show_spinner=False)
def get_screening_universe(cc: str, sector: str) -> pd.DataFrame:
    """
    Restituisce lista ticker filtrata per paese e settore.
    Per USA: filtra S&P 500 Wikipedia per GICS.
    Per altri: usa lista curata + fallback Wikipedia indice locale.
    """
    if cc == "US":
        df = get_sp500_tickers_global()
        if not df.empty:
            gics = SECTOR_TO_GICS_MAP.get(sector, sector)
            filtered = df[df["sector_raw"] == gics].copy()
            if not filtered.empty:
                filtered["sector_raw"] = sector
                return filtered.reset_index(drop=True)
        # Fallback hardcoded US tech/sector tickers se Wikipedia fallisce
        US_FALLBACK = {
            "Tecnologia":["AAPL","MSFT","NVDA","META","GOOGL","AMZN","TSLA","AVGO","ORCL","CRM",
                          "AMD","ADBE","NFLX","QCOM","TXN","INTC","IBM","AMAT","MU","LRCX"],
            "Finanziari":["JPM","BAC","WFC","GS","MS","C","AXP","BLK","SCHW","USB",
                          "PNC","TFC","COF","BK","STT","MET","PRU","AFL","AIG","CB"],
            "Healthcare":["UNH","JNJ","LLY","ABBV","MRK","TMO","ABT","DHR","BMY","AMGN",
                          "PFE","ISRG","SYK","BSX","MDT","ZBH","BDX","EW","HUM","CI"],
            "Energia":   ["XOM","CVX","COP","EOG","SLB","MPC","PSX","VLO","PXD","OXY",
                          "HES","DVN","FANG","HAL","BKR","MRO","APA","CTRA","OVV","PR"],
            "Industriali":["GE","CAT","UNP","RTX","HON","LMT","BA","DE","MMM","GD",
                           "NOC","EMR","ETN","PH","ROK","FDX","UPS","CSX","NSC","WM"],
            "Ciclici":   ["AMZN","TSLA","HD","MCD","NKE","LOW","SBUX","TJX","BKNG","CMG",
                          "ABNB","MAR","HLT","YUM","DHI","LEN","PHM","GM","F","APTV"],
            "Materiali": ["LIN","APD","ECL","SHW","FCX","NEM","NUE","VMC","MLM","PPG",
                          "ALB","MOS","CF","FMC","IFF","CE","EMN","PKG","SEE","AVY"],
            "Utilities": ["NEE","DUK","SO","D","AEP","SRE","EXC","XEL","WEC","ES",
                          "AWK","ETR","FE","AES","PPL","NI","CMS","CNP","EVRG","PNW"],
            "Comunicazioni":["GOOGL","META","NFLX","DIS","CMCSA","T","VZ","CHTR","PARA","EA",
                             "TTWO","MTCH","WBD","FOXA","NWS","IAC","LYV","OMC","IPG","NWSA"],
            "Real Estate":["PLD","AMT","EQIX","CCI","PSA","DLR","O","SPG","WELL","AVB",
                           "EQR","ARE","BXP","VTR","UDR","CPT","MAA","ESS","NNN","KIM"],
            "Beni di prima necessità":["WMT","PG","KO","PEP","COST","PM","MO","CL","KMB","GIS",
                                       "HSY","MKC","CAG","SJM","CHD","HRL","CPB","K","TSN","ADM"],
        }
        tickers = US_FALLBACK.get(sector, [])
        if tickers:
            return pd.DataFrame([{"ticker":t,"name":t,"sector_raw":sector} for t in tickers])
        return pd.DataFrame()
    else:
        # Paesi non-US: usa lista curata
        rows = CURATED_STOCKS_SCREENING.get(cc, [])
        if rows:
            df = pd.DataFrame(rows, columns=["ticker","name","sector_raw"])
...
@st.cache_data(ttl=3600, show_spinner=False)
def fetch_fundamentals_global(ticker: str) -> dict:
    """Scarica fondamentali via yfinance. Cache globale 1h."""
    try:
        t    = yf.Ticker(ticker)
        info = t.info
        fcf      = info.get("freeCashflow")
...
def _page_screening():
    ...
    # (funzioni di screening spostate a livello globale per cache corretta)
    def score_stock(row: dict, medians: dict) -> float:
        ...
        def add(val, med, higher_better=True, weight=1.0):
            ...
            ratio = val / med
            score = (min(ratio, 2.0)/2.0) if higher_better else (min(1/ratio if ratio>0 else 0, 2.0)/2.0)
            points += score * weight; total += weight
    ...
    def color_vs_median(val, median, higher_better=True):
        ...
        return "#16a34a" if (val < median if not higher_better else val > median) else "#dc2626"
    def fmt_mktcap(v):
        if v is None: return "—"
    ...
    sector_list = list(SECTOR_TO_GICS_MAP.keys())
    ...
    with st.spinner(f"Carico universo azioni — {sector} ({country_name})..."):
        universe = get_screening_universe(cc, sector)
    ...
    if "m3_data" not in st.session_state or st.session_state.get("m3_sector_loaded") != f"{cc}_{sector}_{max_tickers}":
        ...
        for i, tkr in enumerate(tickers_all):
            ...
            data_f = fetch_fundamentals_global(tkr)
            if data_f:
                data_f["ticker"] = tkr
