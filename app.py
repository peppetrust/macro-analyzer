...
def render_bottom_nav():
    page = st.session_state.get("page","mattone1")
    regime_ok = st.session_state.get("regime") is not None

    def nav_btn(label, icon, target, enabled=True):
        is_active = page == target
        active_class = "active" if is_active else ""
        disabled_style = "opacity:0.35;pointer-events:none;" if not enabled else ""
        return (
            f'<a class="nav-item {active_class}" style="{disabled_style}" ' +
    ...
    html += nav_btn("Macro",    "🌍", "mattone1", True)
    html += nav_btn("Settori",  "📊", "mattone2", regime_ok)
    html += nav_btn("Screening","🔍", "mattone3", regime_ok)
    html += nav_btn("FairValue","🎯", "mattone4", regime_ok)
    html += '</div>'
...
with st.sidebar:
    ...
    MATTONI ATTIVI<br>📦 1 · Macro ✅<br>📦 2 · Settori ✅<br>📦 3 · Screening ✅<br>📦 4 · Fair Value ✅<br><br>
    FONTI<br>· FRED · World Bank<br>· yfinance · IMF/OCSE
    </div>""",unsafe_allow_html=True)
...
if st.session_state.page == "mattone1":
    # ── NAVBAR MOBILE ──
    _rdy = st.session_state.get("regime") is not None
    _cur = st.session_state.page
    _nav_items = [("🌍 Macro","mattone1",True),("📊 Settori","mattone2",_rdy),("🔍 Screening","mattone3",_rdy),("🎯 Fair Value","mattone4",_rdy)]
    _cols = st.columns(4)
    for _col,(_lbl,_pg,_ok) in zip(_cols,_nav_items):
        with _col:
            if st.button(_lbl, key="nb_"+_pg, use_container_width=True,
                         disabled=not _ok,
                         type="primary" if _cur==_pg else "secondary"):
                st.session_state.page=_pg; st.rerun()
    st.markdown("<hr style='border:none;border-top:1px solid #e5e7eb;margin:2px 0 14px'>", unsafe_allow_html=True)

    st.markdown(f"## 📊 Macro Analyzer — Analisi Top-Down")
    st.markdown('<div style="font-family:DM Mono;font-size:11px;color:#9ca3af;margin-bottom:16px;">MATTONE 1 · STEP 1 DEL FRAMEWORK TOP-DOWN</div>',unsafe_allow_html=True)

    # ── SELECTOR PAESE INLINE (mobile-friendly) ──
    _p_col1, _p_col2 = st.columns([2,1])
    with _p_col1:
        _new_country = st.selectbox("🌍 Seleziona Paese", list(COUNTRIES.keys()),
            index=list(COUNTRIES.keys()).index(st.session_state.country_name),
            key="inline_country_select")
        if _new_country != st.session_state.country_name:
            st.session_state.country_name = _new_country
            st.session_state.cc = COUNTRIES[_new_country]
            st.session_state.regime = None
            st.rerun()
    with _p_col2:
        st.session_state.years_back = st.slider("Anni", 5, 20,
            st.session_state.get("years_back",10), key="inline_years")
