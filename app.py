...
def _page_sectors():
    ...
    _pages    = [("🌍 Macro","macro"),("📊 Settori","sectors"),("🔍 Screening","screening"),("🎯 Fair Value","fairvalue")]
    ...
    for _nc, (_lbl, _pg) in zip(_nav_cols, _pages):
        with _nc:
            ...
            _enabled = True if _pg == "macro" else _regime_ready
...
def _page_screening():
    ...
    with nav2:
        if st.button("▶️ Vai al Mattone 4 — Fair Value", type="primary", use_container_width=True):
            st.session_state.page = "fairvalue"
            st.rerun()
